...
import com.cg.ndp.service.IUserService;
...
public class DonorController {
	...
	public ResponseEntity<List<DonationModel>> getAllDonations(@PathVariable("donorId")int donorId) throws NoSuchDonorException{
		return ResponseEntity.ok(donorService.getAllDonations(donorId));
	}
	
	
	@PutMapping("/{donorId}/modifydonor")
	public ResponseEntity<String> updateDonor(@RequestBody DonorModel donor,@PathVariable("donorId")int donorId)throws NoSuchDonorException {
	...
	public ResponseEntity<String> sendThankYouMail(@RequestBody DonorModel donorModel) {
		ResponseEntity<String>response = null;
		if(donorModel!= null) {
			response = new ResponseEntity<>("Email Password Mail was Sent to Id "+donorModel.getDonorId(),HttpStatus.OK);
		}
